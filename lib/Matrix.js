...
class Matrix {
  ...
  getOldTrainings(numDaysThreshold, today, timeZone) {
    ...
    return this.getAllTrainings().filter((training) => {
      return moment.tz(training.date, Training.DATE_FORMAT, timeZone).isBefore(boundary);
    });
  }
  ...
  static saveData(ellipsis, newValue, cellIndex) {
    const client = require('google-client')(ellipsis);
    const {google} = ellipsis.require('googleapis@36.0.0');
    const sheets = google.sheets('v4');
    const cellRange = `${cellIndex}:${cellIndex}`;
    const updateRange = `${ellipsis.env.SSF_FARM_TRAINING_SHEET_NAME}!${cellRange}`;
    return client.authorize().then(() => {
